...
public class BaseTest {
	...
	/*
	 * @BeforeClass // @Parameters({ "os", "browser" }) public void setup() throws
	 * IOException {
	 * 
	 * // loading properties file FileReader file = new
	 * FileReader(".\\src\\test\\resources\\config.properties"); p = new
	 * Properties(); p.load(file);
	 * 
	...
	// Use below code when individual class is to be Run with Runtime parameter from
	...
	public void runTimesetup() throws IOException {
		...
		if (browserName.contains("chrome")) {
			ChromeOptions options = new ChromeOptions(); // to run the scripts in headless mode
		...
		driver.manage().timeouts().implicitlyWait(Duration.ofSeconds(60));
		// driver.get("https://stag-5-cai.convirza.com/login");
		String appUrl = System.getProperty("appURL") != null ? System.getProperty("appURL") : p.getProperty("appURL");
		// driver.get(p.getProperty("appBetaURL"));
		// driver.get(p.getProperty("appProdURL"));
		driver.get(appUrl);
		driver.manage().window().maximize();
